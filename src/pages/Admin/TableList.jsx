...
const TableList = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/qrlist")
      .then((response) => {
  ...
  const handleToggle = async (qr) => {
    ...
    try {
      // Update the database
      await axios.put(`http://localhost:5000/qrlist/${qr._id}`, {
          ...
          {qrlist.map((qr, index) => (
            <tr key={qr._id} className="text-center">
              ...
              <td className="border border-gray-300 px-4 py-2">
                {qr.active === 1 ? (
                  <a
                    href={`http://localhost:5000/${qr.filename}`}
                    download={`${qr.qrname}.png`}
                    className="cursor-pointer" target="_blank"
                  >
                    <img
                      src={`http://localhost:5000/${qr.filename}`}
                      alt={qr.qrname}
