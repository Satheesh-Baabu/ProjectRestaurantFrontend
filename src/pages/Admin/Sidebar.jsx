...
function Sidebar({ isOpen, toggleSidebar, menuItems, onMenuClick, links }) {
  return (
    ...
    >
      <div className="p-4 font-bold text-lg flex justify-between items-center border-b border-hovercolor">
        <span>MSV Restaurant</span>
        <button className="lg:hidden text-white" onClick={toggleSidebar}>X</button>
      </div>
      <nav className="mt-4">
        <ul>
          {menuItems.map((item, index) => (
            <Link to={links[index]} key={index} onClick={() => onMenuClick(item, index)}>
              <li className="flex items-center space-x-3 p-4 hover:bg-hovercolor transition duration-200">
                <span>{item.icon}</span>
                <span>{item.name}</span>
              </li>
            </Link>
